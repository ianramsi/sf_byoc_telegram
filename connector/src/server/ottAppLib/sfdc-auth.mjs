...
// Salesforce doesn't return an expires_in for JWT bearer tokens, so proactively
// refresh well before typical session-settings timeouts to avoid serving a dead token.
...
export async function getAccessToken(refresh) {
  if (refresh || !cachedAccessToken || Date.now() >= cachedAccessTokenExpiresAt) {
