...
export async function sendSFInboundMessageInteraction(orgId, authorizationContext, channelAddressIdentifier, endUserClientIdentifier, req) {
  ...
  let requestHeader = getInboundMessageRequestHeader(accessToken, orgId, authorizationContext);
  ...
  try {
    let response;
    try {
      response = await axios.post(interactionsApiUrl, formData, requestHeader);
    } catch (error) {
      if (error.response?.data?.code === 10004) {
        logger.info(`Access token invalid (code 10004) for requestId ${requestId}, refreshing and retrying once.`);
        const freshToken = await getAccessToken(true);
        requestHeader = getInboundMessageRequestHeader(freshToken, orgId, authorizationContext);
        response = await axios.post(interactionsApiUrl, formData, requestHeader);
      } else {
        throw error;
      }
