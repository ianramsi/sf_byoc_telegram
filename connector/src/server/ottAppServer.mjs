...
import { handleTelegramWebhook } from './ottAppLib/telegram-webhook.mjs';
import { sendTelegramMessage } from './ottAppLib/telegram-outbound.mjs';
...
async function fetchAndCacheCCDValues() {
  ...
  if(conversationChannelDefinitionData){
    ...
    subscribeToMessagingSessionChangeEvent(sfdcPubSubClient);
  }
}
...
export async function initOttApp(expressApp) {
   ...
   * - Acts as the bridge between Telegram and Salesforce
   *
   * SETUP:
   * 1. Start ngrok tunnel: ngrok http 3000
   * 2. Get public URL: https://xxxxx.ngrok-free.app
   * 3. Register with Telegram Bot API:
   *    curl -X POST "https://api.telegram.org/bot{TOKEN}/setWebhook" \
   *         -d "url=https://xxxxx.ngrok-free.app/api/telegram/webhook"
   *
   * FLOW:
   * When user sends message to @Lori_V01_bot:
   * 1. Telegram Bot API calls this endpoint
   * 2. We parse the Telegram payload
   * 3. Transform to demo connector format
   * 4. Return 200 OK immediately (don't block Telegram)
   ...
   *   "success": true,
   *   "data": {
   *     "message": "Telegram message received and queued for Salesforce processing",
   *     "messageId": "telegram_987654321_1",
   *     "userId": "987654321",
   *     "payload": { ... }
   *   }
   * }
   *
   * SECURITY:
   * - Telegram Bot API includes signature header (see telegram-webhook.mjs)
   * - In production, validate signature to prevent spoofing
   * - Current MVP: trust localhost
   *
   * DEBUGGING:
   * - Monitor logs: Look for "TELEGRAM WEBHOOK RECEIVED" lines
   * - Check parsed fields: messageId, userId, messageText
   * - Verify conversion to sendmessage format
   *
   * ============================================================================
   */
  expressApp.post('/api/telegram/webhook', jsonParser, async (req, res) => {
    try {
      // Call Telegram webhook handler (defined in telegram-webhook.mjs)
      // This does all the heavy lifting: validate, parse, convert
      const responseData = await handleTelegramWebhook(req, settingsCache);

      // Reject unauthenticated requests (missing/invalid secret token header)
      // with 403 — these are not from Telegram, so retry behavior is irrelevant.
      if (responseData.error === 'Invalid signature') {
        res.status(403).json({ success: false, error: 'Forbidden' });
        return;
      }

      // Always return 200 OK to Telegram (within 30 second timeout)
      // This tells Telegram the webhook was received successfully
      // Even if there's a processing error, return 200 to prevent retries
      res.status(200).json(responseData);
    } catch (error) {
      // Catch any unexpected errors (e.g., JSON parsing issues)
      logger.error('Unexpected error in Telegram webhook endpoint:', error);

      // Still return 200 to Telegram, but with error details for our logs
      // Returning 5xx or timing out causes Telegram to retry the webhook
   ...
   *
   * Called by the n8n bot workflow when a Telegram user escalates to an agent
   * (or sends a follow-up message while escalated). n8n cannot POST multipart
   * form-data with a custom boundary (its HTTP node rewrites the boundary),
   * so it delegates the Salesforce Interaction Service call to this connector,
   * which already sends it correctly via FormData.
   *
   * Also caches the Telegram chat id so the outbound leg (agent reply →
   * Telegram) knows where to deliver replies — n8n owns the bot webhook now,
   * so this connector's own /api/telegram/webhook no longer receives traffic.
   *
   * Body: { "chatId": 123456789, "text": "message text" }
   * Auth: X-Connector-Secret header must match N8N_CONNECTOR_SECRET env var.
   * ============================================================================
   */
  expressApp.post('/api/escalate', jsonParser, async (req, res) => {
    try {
      const expectedSecret = process.env.N8N_CONNECTOR_SECRET;
      if (!expectedSecret || req.headers['x-connector-secret'] !== expectedSecret) {
        logger.warn('Escalate endpoint rejected: missing or invalid X-Connector-Secret header');
        res.status(403).json({ success: false, error: 'Forbidden' });
        return;
      }
      ...
      settingsCache.set("telegramChatId", String(chatId));
      ...
      logger.info(`[ESCALATE] n8n escalation request for chatId ${chatId}, publishing to Salesforce...`);
      const sfResponse = await sendSFInboundMessageInteraction(
        settingsCache.get("orgId"),
        settingsCache.get("authorizationContext"),
        settingsCache.get("channelAddressIdentifier"),
        settingsCache.get("endUserClientIdentifier"),
        fakeReq
      );
      logger.info('[ESCALATE] Salesforce publish response:', sfResponse);

      if (sfResponse && sfResponse.success) {
        res.status(200).json(sfResponse);
      } else {
        res.status(502).json({ success: false, error: 'Salesforce interaction failed', details: sfResponse });
      }
    } catch (error) {
      logger.error('Unexpected error in escalate endpoint:', error);
      res.status(500).json({ success: false, error: 'Internal server error' });
    }
  });
                ...
                case messagingConstants.EVENT_PAYLOAD_ENTRY_TYPE.MESSAGE:
                  ...
                  switch (messageType) {
                    ...
                    case messagingConstants.EVENT_PAYLOAD_MESSAGE_TYPE.STATIC_CONTENT_MESSAGE:
                      ...
                      // Step 7b: forward agent reply to the Telegram user
                      sendTelegramMessage(settingsCache.get("telegramChatId"), getFieldValue(payloadFieldObj, 'text'));
                      break;
                    default:
...
/**
 * ============================================================================
 * Session-end detection via Change Data Capture on MessagingSession.
 *
 * Subscribes to the standard CDC channel `/data/MessagingSessionChangeEvent`.
 * When a session for our Telegram channel (ChannelKey === channelAddressIdentifier)
 * shows Status === 'Ended' — whether the agent ended it, it timed out, or it
 * closed via automation — this notifies the Telegram user directly and calls
 * the n8n reset webhook so the chat_state row goes back to 'new'. Without
 * this, the user's next free-text message would silently forward into a
 * closed Salesforce session instead of showing the menu again.
 *
 * MVP note: like the rest of this connector, this assumes a single active
 * conversation (settingsCache "telegramChatId"). Multi-user support needs a
 * conversationIdentifier -> chatId map instead (see handoff doc task list).
 * ============================================================================
 */
const MESSAGING_SESSION_CHANGE_EVENT_CHANNEL = '/data/MessagingSessionChangeEvent';

async function subscribeToMessagingSessionChangeEvent(sfdcPubSubClient) {
  try {
    if (!sfdcPubSubClient) {
      return;
    }

    const subscription = subscribe(sfdcPubSubClient, MESSAGING_SESSION_CHANGE_EVENT_CHANNEL);
    const topicSchema = await getEventSchema(sfdcPubSubClient, MESSAGING_SESSION_CHANGE_EVENT_CHANNEL);
    logger.info('Subscribed to MessagingSessionChangeEvent for session-end detection.');

    subscription.on('data', (data) => {
      if (!data.events) {
        return;
      }
      const parsedEvents = data.events.map((event) => parseEvent(topicSchema, event));

      parsedEvents.forEach(async (event) => {
        try {
          const payload = event.payload;
          const statusField = getFieldValue(payload, 'Status');
          const status = statusField && statusField.string ? statusField.string : null;
          logger.info('MessagingSessionChangeEvent received: ', { status });

          if (status !== 'Ended') {
            return;
          }

          // CDC change events only ship fields that changed in that specific
          // update, so ChannelKey (set once at session creation, not on the
          // Status-only "Ended" delta) is expected to come through null here.
          // Only reject when the field IS present and mismatched; a missing
          // field falls through (acceptable for this single-channel MVP).
          const channelKeyField = getFieldValue(payload, 'ChannelKey');
          const channelKey = channelKeyField && channelKeyField.string ? channelKeyField.string : null;
          const channelAddressIdFromSettings = settingsCache.get('channelAddressIdentifier');
          if (channelKey && channelKey !== channelAddressIdFromSettings) {
            return;
          }

          const endedByTypeField = getFieldValue(payload, 'EndedByType');
          const endedByType = endedByTypeField && endedByTypeField.string ? endedByTypeField.string : 'Unknown';
          logger.info(`MessagingSession ended (endedByType=${endedByType}) for our Telegram channel.`);

          const telegramChatId = settingsCache.get('telegramChatId');
          if (!telegramChatId) {
            logger.warn('MessagingSession ended but no telegramChatId cached — cannot notify or reset.');
            return;
          }

          await sendTelegramMessage(telegramChatId, 'This conversation has ended. Send anything to see the menu again.');
          await notifyN8nChatEnded(telegramChatId);
        } catch (err) {
          logger.error('Error handling MessagingSessionChangeEvent: ', err);
        }
      });
    });
    subscription.on('end', () => {
      logger.info('gRPC stream ended (MessagingSessionChangeEvent)');
    });
    subscription.on('error', (err) => {
      logger.error('gRPC stream error (MessagingSessionChangeEvent): ', JSON.stringify(err));
    });
    subscription.on('status', (status) => {
      logger.info('gRPC stream status (MessagingSessionChangeEvent): ', status);
    });
  } catch (err) {
    logger.error('Fatal error in subscribeToMessagingSessionChangeEvent: ', err);
  }
}

/**
 * Tells the n8n workflow to reset a chat's state row back to 'new' after the
 * Salesforce session for it ended. Uses the same shared secret already used
 * for the reverse direction (n8n -> connector /api/escalate).
 */
async function notifyN8nChatEnded(chatId) {
  const resetUrl = process.env.N8N_RESET_WEBHOOK_URL;
  const secret = process.env.N8N_CONNECTOR_SECRET;
  if (!resetUrl || !secret) {
    logger.warn('N8N_RESET_WEBHOOK_URL or N8N_CONNECTOR_SECRET not configured — cannot reset n8n chat state.');
    return;
  }
  try {
    await axios.post(resetUrl, { chatId }, { headers: { 'X-Connector-Secret': secret } });
    logger.info(`Notified n8n to reset chat state for chatId ${chatId}`);
  } catch (err) {
    logger.error('Failed to notify n8n reset webhook: ', err.response?.data || err.message);
  }
}
